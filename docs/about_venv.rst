...
Virtual environments allow you to isolate Edu's library dependencies, cog dependencies and python
binaries from the rest of your system. There is no performance overhead to using virtual environment
and it saves you from a lot of troubles during setup. It also makes sure Edu and its dependencies
are installed to a predictable location which makes uninstalling Edu as simple as removing a single folder,
...
--------------------------------------------
If you are running multiple instances of Edu on the same machine, you have the option of either
...
- When updating Edu, you will only need to update it once for all instances (however you will still need to restart all instances for the changes to take effect)

...
- Eduuced the number of API calls made to the storage APIs (:issue:`3910`)
...
- Edu's start up message now shows storage type (:issue:`3935`)
...
- Added `bot.set_prefixes() <EduBase.set_prefixes()>` method that allows developers to set global/server prefixes (:issue:`3890`)
...
- Edu now includes information on how to update when sending information about being out of date (:issue:`3744`)
...
| **Important:**
| If you're using RPC, please see the full annoucement about current state of RPC in main Edu server
...
- Edu now inherits from `discord.ext.commands.AutoShardedBot` for better compatibility with code expecting d.py bot (:issue:`3822`)
...
- Improved navigation in `document about updating Edu <update_red>` (:issue:`3856`, :issue:`3849`)
...
- Various optimizations
  ...
  - Eduuced calls to data backend when loading bot's commands (:issue:`3764`)
  - Eduuced calls to data backend when showing help for cogs/commands (:issue:`3766`)
  ...
  - Eduuced the bot load for messages starting with a prefix when fuzzy search is disabled (:issue:`3718`)
...
- Updated ``leagueoflegends`` list with new changes to League of Legends (`b8ac70e <https://github.com/Cog-Creators/Edu-DiscordBot/commit/b8ac70e59aa1328f246784f14f992d6ffe00d778>`_)
...
- Added `document about updating Edu <update_red>` (:issue:`3790`)
...
- "Outdated" field no longer shows in ``[p]info`` when Edu is up-to-date (:issue:`3730`)
...
- ``[p]set locale`` allows any valid locale now, not just locales for which Edu has translations (:issue:`3676`, :issue:`3596`)
...
- ``[p]cog checkforupdates`` now includes information about cogs that can't be installed due to Edu/Python version requirements (:issue:`3678`, :issue:`3448`)
...
- Deprecation warnings issued by Edu now use correct stack level so that the cog developers can find the cause of them (:issue:`3644`)
...
- Fix load error for users that updated Edu from version lower than 3.1 to version 3.2 or newer (:issue:`3617`)

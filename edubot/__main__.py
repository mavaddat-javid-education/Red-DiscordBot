...
from edubot import _early_init, __version__

_early_init()

import edubot.logging
from edubot.core.bot import Edu, ExitCodes
from edubot.core.cli import interactive_config, confirm, parse_cli_flags
from edubot.setup import get_data_dir, get_name, save_config
from edubot.core import data_manager, drivers
from edubot.core._sharedlibdeprecation import SharedLibImportWarner
...
def list_instances():
    if not data_manager.config_file.exists():
        print(
            ...
            "using `edubot-setup` before trying to run the bot!"
        )
        sys.exit(1)
    else:
...
async def _edit_owner(red, owner, no_prompt):
    if owner:
        ...
        print(
            "Remember:\n"
            "ONLY the person who is hosting Edu should be owner."
...
def handle_edit(cli_flags: Namespace):
    ...
    red = Edu(cli_flags=cli_flags, description="Edu V3", dm_help=None)
    try:
...
async def run_bot(red: Edu, cli_flags: Namespace) -> None:
    """
    This runs the bot.
    ...
    if cli_flags.token:
        ...
        token = os.environ.get("EDU_TOKEN", None)
        if not token:
    ...
    try:
        ...
        console.print(
            "Edu requires all Privileged Intents to be enabled.\n"
...
def edu_exception_handler(red, edu_task: asyncio.Future):
    """
    This is set as a done callback for Edu
...
def main():
    ...
    try:
        ...
        if os.name != "nt":
            ...
            # At least it's not a eduundant handler...
        ...
        r_exc_handler = functools.partial(edu_exception_handler, red)
        fut.add_done_callback(r_exc_handler)
        loop.run_forever()
    except KeyboardInterrupt:
        # We still have to catch this here too. (*joy*)
        log.warning("Please do not use Ctrl+C to Shutdown Edu! (attempting to die gracefully...)")

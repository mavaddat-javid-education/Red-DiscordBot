...
from edubot.core.commands import UserInputOptional
from edubot.core.data_manager import cog_data_path
from edubot.core.i18n import Translator
from edubot.core.utils import AsyncIter
from edubot.core.utils.chat_formatting import bold, pagify
from edubot.core.utils.menus import DEFAULT_CONTROLS, menu
from edubot.core.utils.predicates import MessagePredicate
...
class PlaylistCommands(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def command_playlist_copy(
        ...
        **Guild** can be one of the following:
        ...
        ​ ​ ​ ​ `[p]playlist copy MyPersonalPlaylist --from-scope user --to-author Draper#6666 --to-scope Guild --to-guild Edu - Discord Bot`
        """
        if self.playlist_api is None:
            return await self.send_embed_msg(
                ctx,
    ...
    async def command_playlist_download(
        ...
        *,
        ...
        Edu v2-compatible playlists can be generated by passing True
        for the v2 variable.
    ...
    async def command_playlist_upload(
        ...
        **Guild** can be one of the following:
        ...
        async with ctx.typing():
            ...
            if file_suffix != "txt":
                return await self.send_embed_msg(
                    ctx, title=_("Only Edu playlist files can be uploaded.")
                )
            try:

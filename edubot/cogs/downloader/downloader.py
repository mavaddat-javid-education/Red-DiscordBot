...
import discord
from edubot.core import checks, commands, Config, version_info as red_version_info
from edubot.core.bot import Edu
from edubot.core.data_manager import cog_data_path
from edubot.core.i18n import Translator, cog_i18n
from edubot.core.utils.chat_formatting import box, pagify, humanize_list, inline
from edubot.core.utils.menus import start_adding_reactions
from edubot.core.utils.predicates import MessagePredicate, ReactionPredicate
...
class Downloader(commands.Cog):
    ...
    async def _available_updates(
        ...
        # Eduuces diff requests to a single dict with no repeats
    ...
    async def _install_requirements(self, cogs: Iterable[Installable]) -> Tuple[str, ...]:
        ...
        # Eduuces requirements to a single list with no repeats
    ...
    async def _cog_reinstallreqs(self, ctx: commands.Context) -> None:
        """
        This command should not be used unless Edu specifically asks for it.

        This command will reinstall cog requirements and shared libraries for all installed cogs.

        Edu might ask the owner to use this when it clears contents of the lib folder
    ...
    def _filter_incorrect_cogs(
        ...
        if outdated_bot_version:
            message += (
                _(
                    "\nThese cogs require different Edu version"
                    " than you currently have ({current_version}): "
                )
                if len(outdated_bot_version) > 1
                else _(
                    "\nThis cog requires different Edu version than you currently "
    ...
    async def findcog(self, ctx: commands.Context, command_name: str) -> None:
        ...
        **Arguments**
        ...
        if cog:
            ...
            elif cog.__module__.startswith("edubot."):  # core commands or core cog
                made_by = "Cog Creators"
                repo_url = "https://github.com/mavaddat-javid-education/Edu-DiscordBot"

...
class EduBase(
    ...
    def __init__(self, *args, cli_flags=None, bot_dir: Path = Path.cwd(), **kwargs):
        ...
        if "owner_id" in kwargs:
            raise RuntimeError("Edu doesn't accept owner_id kwarg, use owner_ids instead.")
        ...
        self._help_formatter = commands.help.EduHelpFormatter()EduH
    ...
    def set_help_formatter(self, formatter: commands.help.HelpFormatterABC):
        ...
        if type(self._help_formatter) is commands.help.EduHelpFormatter:
    ...
    def reset_help_formatter(self):
        ...
        """
        self._help_formatter = commands.help.EduHelpFormatter()
    ...
    def before_invoke(self, coro: T_BIC) -> T_BIC:
        """
        Overridden decorator method for Edu's ``before_invoke`` behavior.
    ...
    async def before_identify_hook(self, shard_id, *, initial=False):
        ...
        self.dispatch("edu_before_identify", shard_id, initial)
    ...
    async def allowed_by_whitelist_blacklist(
        ...
        *,
        ...
        as considered by Edu's whitelist and blacklist.
        ...
        if guild:
            ...
            # Converting to a set eduuces the total lookup time in section
            if mocked:
    ...
    async def send_help_for(
        ...
        *,
        ...
        """
        Invokes Edu's helpformatter for a given context and object.
        """
    ...
    async def set_shared_api_tokens(self, service_name: str, **tokens: str):
        ...
        **tokens
        ...
        async with self._config.custom(SHARED_API_TOKENS, service_name).all() as group:
            group.update(tokens)
        self.dispatch("edu_api_tokens_update", service_name, MappingProxyType(group))
    ...
    async def remove_shared_api_services(self, *service_names: str):
        ...
        *service_names: str
        ...
        for service in service_names:
            self.dispatch("edu_api_tokens_update", service, MappingProxyType({}))
    ...
    def add_command(self, command: commands.Command) -> None:
        if not isinstance(command, commands.Command):
            raise RuntimeError("Commands must be instances of `edubot.core.commands.Command`")
    ...
    async def handle_data_deletion_request(
        ...
        *,
        ...
        """
        This tells each cog and extension, as well as any APIs in Edu
        to go delete data

        Calling this should be limited to interfaces designed for it.

        See ``edubot.core.commands.Cog.delete_data_for_user``
    ...
    async def _handle_data_deletion_request(
        ...
        *,
        ...
        extension_handlers = {
            ...
            if (handler := getattr(extension, "edu_delete_data_for_user", None))
        }

        cog_handlers = {
            cog_qualname: cog.edu_delete_data_for_user for cog_qualname, cog in self.cogs.items()
        }

        special_handlers = {
            "Edu Core Modlog API": modlog._process_data_deletion,
            "Edu Core Bank API": bank._process_data_deletion,
            "Edu Core Bot Data": self._core_data_deletion,
        }
        ...
        async def wrapper(func, stype, sname):
            ...
            except commands.commands.EduUnhandledAPI:

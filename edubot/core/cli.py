...
async def interactive_config(red, token_set, prefix_set, *, print_header=True):
    ...
    if print_header:
        print("Edu - Discord Bot | Configuration process\n")
...
def parse_cli_flags(args):
    parser = argparse.ArgumentParser(
        description="Edu - Discord Bot", usage="edubot <instance_name> [arguments]"
    )
    parser.add_argument("--version", "-V", action="store_true", help="Show Edu's current version")
    parser.add_argument("--debuginfo", action="store_true", help="Show debug information.")
    parser.add_argument(
        "--list-instances",
        action="store_true",
        help="List all instance names setup with 'edubot-setup'",
    )
    parser.add_argument(
        "--edit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "to the system that is hosting Edu should be  "
    ...
    parser.add_argument(
        "--no-cogs", action="store_true", help="Starts Edu with no cogs loaded, only core"
    )
    parser.add_argument(
        "--load-cogs",
        type=str,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Makes Edu quit with code 0 just before the "
    ...
    parser.add_argument("--token", type=str, help="Run Edu with the given token.")
    parser.add_argument(
        "--no-instance",
        action="store_true",
        help=(
            "Run Edu without any existing instance. "
            "The data will be saved under a temporary folder "
            "and deleted on next system restart."
        ),
    )
    parser.add_argument(
        "instance_name", nargs="?", help="Name of the bot instance created during `edubot-setup`."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        " as Edu is not prepared to work without all intents.\n"

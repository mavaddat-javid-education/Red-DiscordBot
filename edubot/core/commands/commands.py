...
RESERVED_COMMAND_NAMES = (
    "cancel",  # reserved due to use in ``edubot.core.utils.MessagePredicate``
)
...
class EduUnhandledAPI(Exception):
...
class Command(CogCommandMixin, DPYCommand):
    ...
    .. warning::
        ...
        None of your methods should start with ``edu_`` or
    ...
    def callback(self, function):
        ...
        for key, value in self.params.items():
            ...
            if not self.ignore_optional_for_conversion:
                continue  # eduuces indentation compared to alternative
    ...
    def error(self, coro):
        ...
        Edu's global error handler will ignore commands with a registered error handler.

        To have red handle specific errors with the default behavior,
        call ``Edu.on_command_error`` with ``unhandled_by_cog`` set to True.
...
class GroupMixin(discord.ext.commands.GroupMixin):
    """Mixin for `Group` and `Edu` classes.
...
class CogMixin(CogGroupMixin, CogCommandMixin):
    ...
    async def edu_get_data_for_user(self, *, user_id: int) -> MutableMapping[str, io.BytesIO]:
        """

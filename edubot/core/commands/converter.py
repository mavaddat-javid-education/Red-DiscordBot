...
class _GuildConverter(discord.Guild):
    ...
    .. deprecated-removed:: 3.4.8 60
        ``GuildConverter`` is now only provided within ``edubot.core.commands`` namespace.
    """
...
def __getattr__(name: str, *, stacklevel: int = 2) -> Any:
    ...
    if name == "GuildConverter":
        ...
        from edubot.core.utils._internal_utils import deprecated_removed

        deprecated_removed(
            "`GuildConverter` from `edubot.core.commands.converter` namespace",
            "3.4.8",
            60,
            "Use `GuildConverter` from `edubot.core.commands` namespace instead.",
            stacklevel=2,
        )

...
# This is not an unreasonable volume on some already existing Edu instances,
...
__all__ = ["edu_help", "EduHelpFormatter", "HelpSettings", "HelpFormatterABC"]
...
class HelpFormatterABC(abc.ABC):
    ...
    .. note::
        ...
        Edu will not initialize a formatter for you,
        and must be passed an initialized formatter.

        If you want to use Edu's existing settings, use ``HelpSettings.from_context``
...
class EduHelpFormatter(HelpFormatterABC):
    """
    Edu's help implementation
    ...
    async def help_filter_func(
        ...
        for obj in objects:
            if verify_checks and not show_hidden:
                # Default Edu behavior, can_see includes a can_run check.
...
async def edu_help(ctx: Context, *, thing_to_get_help_for: str = None):
    """
    I need somebody

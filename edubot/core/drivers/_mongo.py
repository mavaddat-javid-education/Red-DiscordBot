...
class MongoDriver(BaseDriver):
    ...
    async def initialize(cls, **storage_details) -> None:
        if motor is None:
            raise errors.MissingExtraRequirements(
                "Edu must be installed with the [mongo] extra to use the MongoDB driver"
            )
        ...
        if num_pkeys >= identifier_data.primary_key_len:
            ...
            try:
                await mongo_collection.update_one(
                    {"_id": {"EDU_uuid": uuid, "EDU_primary_key": primary_key}},
                    update=update_stmt,
                    upsert=True,
                )
            ...
            async with await self._conn.start_session() as session:
                ...
                try:
                    ...
                    async for document in mongo_collection.find(pkey_filter, session=session):
                        pkey = document["_id"]["EDU_primary_key"]
                        new_document = value
                        try:
                            ...
                            to_delete.append({"_id": {"EDU_uuid": uuid, "EDU_primary_key": pkey}})
                        else:
                            _filter = {"_id": {"EDU_uuid": uuid, "EDU_primary_key": pkey}}
    ...
    def generate_primary_key_filter(self, identifier_data: IdentifierData):
        uuid = self._escape_key(identifier_data.uuid)
        primary_key = list(map(self._escape_key, self.get_primary_key(identifier_data)))
        ret = {"_id.EDU_uuid": uuid}
        if len(identifier_data.identifiers) > 0:
            ret["_id.EDU_primary_key"] = primary_key
        elif len(identifier_data.primary_key) > 0:
            for i, key in enumerate(primary_key):
                keyname = f"_id.EDU_primary_key.{i}"
                ret[keyname] = key
        else:
            ret["_id.EDU_primary_key"] = {"$exists": True}
        return ret
    ...
    def generate_documents_to_insert(
        ...
        if num_missing_pkeys == 1:
            for pkey, document in data.items():
                document["_id"] = {"EDU_uuid": uuid, "EDU_primary_key": primary_keys + [pkey]}
                yield document
        else:
    ...
    async def aiter_cogs(cls) -> AsyncIterator[Tuple[str, str]]:
        ...
        for collection_name in await db.list_collection_names():
            ...
            if not len(parts) == 2:
                continue
            cog_name = parts[0]
            for cog_id in await db[collection_name].distinct("_id.EDU_uuid"):
    ...
    async def delete_all_data(
        ...
        if interactive is True and drop_db is None:
            print(
                ...
                " 2. Delete all of Edu's data within this database, without dropping the database "
                "itself."
            )
            options = ("1", "2")
            while True:

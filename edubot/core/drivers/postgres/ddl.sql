...
CREATE SCHEMA IF NOT EXISTS edu_config;edu_
CREATE SCHEMA IF NOT EXISTS red_utils;

DO $$
BEGIN
  PERFORM 'edu_config.identifier_data'::regtype;
EXCEPTION
  WHEN UNDEFINED_OBJECT THEN
    CREATE TYPE edu_config.identifier_data AS (
      cog_name text,
      cog_id text,
      category text,
      pkeys text[],
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_config.maybe_create_table(edu_
  ...
  BEGIN
    IF NOT schema_exists THEN
      PERFORM edu_config.create_schema(id_data.cog_name, id_data.cog_id);
    END IF;
    IF NOT table_exists THEN
      PERFORM edu_config.create_table(id_data);
    END IF;
  END;
$$;
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_config.create_schema(new_cog_name text, new_cog_id text, OUT schemaname text)
    RETURNS text
    LANGUAGE 'plpgsql'
  AS $$
  BEGIN
    ...
    INSERT INTO edu_config.edu_cogs AS t VALUES(new_cog_name, new_cog_id, schemaname)
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_config.create_table(id_data edu_config.identifier_data)
    RETURNS void
    LANGUAGE 'plpgsql'
  AS $$
  DECLARE
    ...
    pkey_columns CONSTANT text := edu_utils.gen_pkey_columns(1, id_data.pkey_len);edu_edu_
...
CREATE OR REPLACE FUNCTION
  ...
  DECLARE
    ...
    pkey_type CONSTANT text := edu_utils.get_pkey_type(id_data.is_custom);edu_
  ...
  BEGIN
    ...
    ELSIF num_missing_pkeys = 1 THEN
      ...
      USING id_data.pkeys;
    ELSE
      -- Multiple missing primary keys: we must use our custom edu_utils.jsonb_object_agg2()
      -- aggregate function.
      missing_pkey_columns := edu_utils.gen_pkey_columns_casted(num_pkeys + 1, id_data.pkey_len);

      EXECUTE format(
        'SELECT edu_utils.jsonb_object_agg2(json_data, %s) FROM %I.%I WHERE %s',
...
CREATE OR REPLACE FUNCTION
  ...
  BEGIN
    PERFORM edu_config.maybe_create_table(id_data);

    IF num_missing_pkeys = 0 THEN
      -- Setting all or part of a document
      new_document := edu_utils.jsonb_set2('{}', new_value, VARIADIC id_data.identifiers);
      ...
      EXECUTE format(
        ...
        SET
          json_data = edu_utils.jsonb_set2(t.json_data, $3, VARIADIC $4)
        $query$,
        schemaname,
        id_data.category,
        pkey_placeholders,
    ...
    ELSE
      ...
      whereclause := edu_utils.gen_whereclause(num_pkeys, pkey_type);edu_
      missing_pkey_columns := red_utils.gen_pkey_columns_casted(
        num_pkeys + 1, id_data.pkey_len, pkey_type);
      pkey_column_definitions := edu_utils.gen_pkey_column_definitions(num_pkeys + 1, id_data.pkey_len);
      ...
      EXECUTE format(
        ...
        INSERT INTO %I.%I AS t
          SELECT %s, json_data
          FROM edu_utils.generate_rows_from_object($2, $3) AS f(%s, json_data jsonb)
        ON CONFLICT ON CONSTRAINT %I DO UPDATE
        SET
...
CREATE OR REPLACE FUNCTION
  ...
  DECLARE
    ...
    pkey_type CONSTANT text := edu_utils.get_pkey_type(id_data.is_custom);
  ...
  BEGIN
    IF num_identifiers > 0 THEN
      ...
      whereclause := edu_utils.gen_whereclause(num_pkeys, pkey_type);
    ...
    ELSE
      ...
      DELETE FROM edu_config.edu_cogs
...
CREATE OR REPLACE FUNCTION
  ...
  BEGIN
    ...
    IF existing_document IS NULL THEN
      ...
      new_document := edu_utils.jsonb_set2('{}', result, VARIADIC id_data.identifiers);edu_
    ...
    ELSE
      ...
      new_document := edu_utils.jsonb_set2(
...
CREATE OR REPLACE FUNCTION
  ...
  BEGIN
    ...
    IF existing_document IS NULL THEN
      ...
      new_document := edu_utils.jsonb_set2('{}'::jsonb, result, id_data.identifiers);edu_
      pkey_placeholders := red_utils.gen_pkey_placeholders(id_data.pkey_len, pkey_type);
    ...
    ELSE
      ...
      new_document := edu_utils.jsonb_set2(existing_document, result, id_data.identifiers);
...
CREATE OR REPLACE FUNCTION
  /*
   * Delete all schemas listed in the edu_config.edu_cogs table.
   */
  edu_config.delete_all_schemas()
    RETURNS void
    LANGUAGE 'plpgsql'
  AS $$
  DECLARE
    cog_entry record;
  BEGIN
    FOR cog_entry IN SELECT * FROM edu_config.edu_cogs t LOOP
      EXECUTE format('DROP SCHEMA %I CASCADE', cog_entry.schemaname);
    END LOOP;
    -- Clear out edu_config.edu_cogs tableedu_edu_
...
CREATE OR REPLACE FUNCTION
   ...
   * of a non-document type.
   */
  edu_utils.jsonb_set2(target jsonb, new_value jsonb, VARIADIC identifiers text[])
    RETURNS jsonb
    LANGUAGE 'plpgsql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_utils.generate_rows_from_object(object jsonb, num_missing_pkeys integer)
  ...
  BEGIN
    ...
    ELSE
      ...
      column_definitions := edu_utils.gen_pkey_column_definitions(2, num_missing_pkeys);
      ...
      FOR pair IN SELECT * FROM jsonb_each(object) LOOP
        RETURN QUERY
          EXECUTE format(
            $query$
            SELECT $1 AS key_1, *
            FROM edu_utils.generate_rows_from_object($2, $3)
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_utils.gen_pkey_placeholders(num_pkeys integer, pkey_type text DEFAULT 'text')
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_utils.gen_whereclause(num_pkeys integer, pkey_type text)
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_utils.gen_pkey_columns(start integer, stop integer)
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   * to the given type.
   */
  edu_utils.gen_pkey_columns_casted(start integer, stop integer, pkey_type text DEFAULT 'text')
    RETURNS text
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
CREATE OR REPLACE FUNCTION
   ...
   */
  edu_utils.gen_pkey_column_definitions(
...
CREATE OR REPLACE FUNCTION
  edu_utils.get_pkey_type(is_custom boolean)
    RETURNS TEXT
    LANGUAGE 'sql'
    IMMUTABLE
    PARALLEL SAFE
...
DROP AGGREGATE IF EXISTS edu_utils.jsonb_object_agg2(jsonb, VARIADIC text[]);
CREATE AGGREGATE
  /*
   ...
   * reasons.
   */
  edu_utils.jsonb_object_agg2(json_data jsonb, VARIADIC primary_keys text[]) (edu_

...
class PostgresDriver(BaseDriver):
    ...
    async def initialize(cls, **storage_details) -> None:
        if asyncpg is None:
            raise errors.MissingExtraRequirements(
                "Edu must be installed with the [postgres] extra to use the PostgreSQL driver"
            )
    ...
    def get_config_details():
        ...
        host = (
            input(
                ...
                f"If left blank, Edu will try the following, in order:\n"
        ...
        user = (
            input(
                ...
                " - The OS name of the user running Edu (ident/peer authentication).\n"
                "> "
            )
            or None
        )
        ...
        database = (
            input(
                ...
                " - The OS name of the user running Edu.\n"
                "> "
            )
            or None
        )
    ...
    async def set(self, identifier_data: IdentifierData, value=None):
        try:
            await self._execute(
                "SELECT edu_config.set($1, $2::jsonb)",
                encode_identifier_data(identifier_data),
                json.dumps(value),
            )
    ...
    async def clear(self, identifier_data: IdentifierData):
        try:
            await self._execute(
                "SELECT edu_config.clear($1)", encode_identifier_data(identifier_data)
            )
        except asyncpg.UndefinedTableError:
            pass
    ...
    async def aiter_cogs(cls) -> AsyncIterator[Tuple[str, str]]:
        query = "SELECT cog_name, cog_id FROM edu_config.edu_cogs"
        log.invisible(query)
    ...
    async def delete_all_data(cls, *, drop_db: Optional[bool] = None, **kwargs) -> None:
        ...
        if drop_db is True:
            print(
                ...
                " We will delete all of Edu's data within this database,"

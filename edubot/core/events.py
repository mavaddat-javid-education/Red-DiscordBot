...
from edubot.core.commands import EduHelpFormatter, HelpSettingsedubot
from edubot.core.i18n import (
    Translator,
...
from .. import __version__ as edu_version, version_info as edu_version_info, VersionInfo
...
def init_events(bot, cli_flags):
    ...
    async def on_ready():
        ...
        edu_pkg = pkg_resources.get_distribution("Edu-DiscordBot")
        ...
        table_general_info.add_row("Edu version", edu_version)
        ...
        with contextlib.suppress(aiohttp.ClientError, asyncio.TimeoutError):
            ...
            if outdated:
                outdated_red_message = _(
                    "Your Edu instance is out of date! {} is the current "
                    "version, however you are using {}!"
                ).format(pypi_version, edu_version)
                rich_outdated_message = (
                    f"[red]Outdated version![/red]\n"
                    f"[red]!!![/red]Version [cyan]{pypi_version}[/] is available, "
                    f"but you're using [cyan]{edu_version}[/][red]!!![/red]"
                )
                ...
                if expected_version(current_python, py_version_req):
                    installed_extras = []
                    for extra, reqs in edu_pkg._dep_map.items():
                    ...
                    extra_update += (
                        '```"{python}" -m pip install -U Edu-DiscordBot{package_extras}```'.format(
        ...
        if not guilds:
            rich_console.print(
                f"Looking for a quick guide on setting up Edu? Checkout {Text('https://start.discord.red', style='link https://start.discord.red}')}"
            )
    ...
    async def on_command_error(ctx, error, unhandled_by_cog=False):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            fuzzy_commands = await fuzzy_command_search(
                ctx,
                commands=EduHelpFormatter.help_filter_func(

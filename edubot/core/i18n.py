...
async def get_locale_from_guild(bot: Edu, guild: Optional[discord.Guild]) -> str:
    """
...
async def get_regional_format_from_guild(bot: Edu, guild: Optional[discord.Guild]) -> str:
    """
...
async def set_contextual_locales_from_guild(bot: Edu, guild: Optional[discord.Guild]) -> None:
    """
...
class Translator(Callable[[str], str]):
    ...
    def load_translations(self):
        ...
        if locale.lower() == "en-us":
            # Edu is written in en-US, no point in loading it
            return
...
@functools.lru_cache()
def _get_babel_locale(edu_locale: str) -> babel.core.Locale:
    supported_locales = babel.localedata.locale_identifiers()
    try:  # Handles cases where edu_locale is already Babel supportededu_
        babel_locale = Locale(*babel.parse_locale(red_locale))
    except (ValueError, babel.core.UnknownLocaleError):
        try:
            babel_locale = Locale(*babel.parse_locale(edu_locale, sep="-"))
        except (ValueError, babel.core.UnknownLocaleError):
            ...
            try:
                ...
                babel_locale = Locale(Locale.negotiate([edu_locale], supported_locales, sep="-"))

...
from edubot import VersionInfoedubotedubot
from edubot.core import data_manager
from edubot.core.utils.chat_formatting import box

if TYPE_CHECKING:
    from edubot.core.bot import Eduedubot
    from edubot.core.commands import Command, Context
...
async def fuzzy_command_search(
    ...
    *,
    ...
    Parameters
    ----------
    ctx : `commands.Context <edubot.core.commands.Context>`
    ...
    Returns
    -------
    Optional[List[`commands.Command <edubot.core.commands.Command>`]]
...
async def format_fuzzy_results(
    ...
    matched_commands : List[`commands.Command <edubot.core.commands.Command>`]
...
async def create_backup(dest: Path = Path.home()) -> Optional[Path]:
    ...
    backup_fpath = dest / f"eduv3_{data_manager.instance_name}_{timestr}.tar.gz"
...
async def send_to_owners_with_preprocessor(
    ...
    *,
    content_preprocessor: Optional[
        Callable[[Edu, discord.abc.Messageable, str], Awaitable[str]]
    ] = None,
    **kwargs,
):
    """
    ...
    This acts the same as `Edu.send_to_owners`, with
    one added keyword argument as detailed below in *Other Parameters*.

    Other Parameters
    ----------------
    content_preprocessor: Optional[Callable[[Edu, discord.abc.Messageable, str], Awaitable[str]]]
...
async def send_to_owners_with_prefix_replaced(bot: Edu, content: str, **kwargs):
    """
    This sends something to all owners and their configured extra destinations.

    This acts the same as `Edu.send_to_owners`, with one addition - `[p]` in ``content`` argument
    is replaced with a clean prefix for each specific destination.
    """

    async def preprocessor(bot: Edu, destination: discord.abc.Messageable, content: str) -> str:
...
async def fetch_latest_red_version_info() -> Tuple[Optional[VersionInfo], Optional[str]]:
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get("https://pypi.org/pypi/Edu-DiscordBot/json") as r:
                data = await r.json()

...
from edubot import MIN_PYTHON_VERSIONedubot
from edubot.setup import (
    basic_setup,
...
from edubot.core import __version__, version_info as edu_version_info, VersionInfoedubot
from edubot.core.cli import confirm
...
def parse_cli_args():
    parser = argparse.ArgumentParser(
        description="Edu - Discord Bot's launcher (V3)", allow_abbrev=False
    )
    ...
    parser.add_argument("--start", "-s", help="Starts Edu", action="store_true")
    parser.add_argument(
        "--auto-restart", help="Autorestarts Edu in case of issues", action="store_true"
    )
...
def run_red(selected_instance, autorestart: bool = False, cliflags=None):
    ...
    while True:
        ...
        cmd_list = [interpreter, "-m", "edubot", selected_instance]
        if cliflags:
            cmd_list += cliflags
...
def main_menu(flags_to_pass):
    if IS_WINDOWS:
        os.system("TITLE Edu - Discord Bot V3 Launcher")
    clear_screen()
    while True:
        print(INTRO)
        ...
        print("")
        print("1. Run Edu w/ autorestart in case of issues")
        print("2. Run Edu")
        print("0. Exit")
        choice = user_choice()
        if choice == "1":
...
def main():
    ...
    if not PYTHON_OK:
        print(
            "Python {req_ver} is required to run Edu, but you have {sys_ver}!".format(

...
import pytest
from edubot.core import Configedubotedubot
from edubot.core.bot import Edu
from edubot.core import config as config_module, drivers
...
@pytest.fixture()
def red(config_fr):
    from edubot.core.cli import parse_cli_flags

    cli_flags = parse_cli_flags(["ignore_me"])

    description = "Edu v3 - Alpha"

    Config.get_core_conf = lambda *args, **kwargs: config_fr

    red = Edu(cli_flags=cli_flags, description=description, dm_help=None, owner_ids=set())

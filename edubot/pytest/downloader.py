...
from edubot.cogs.downloader.repo_manager import RepoManager, Repo, ProcessFormatteredubot
from edubot.cogs.downloader.installable import Installable, InstalledModule
...
async def _session_git_repo(tmp_path_factory, event_loop):
    ...
    repo = Repo(name="edubot-testrepo", url="", branch="master", commit="", folder_path=repo_path)
...
async def cloned_git_repo(_session_git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "edubot-cloned_testrepo"
    repo = Repo(
        name="edubot-testrepo",
...
async def git_repo_with_remote(git_repo, tmp_path, event_loop):
    ...
    repo_path = tmp_path / "edubot-testrepo_with_remote"
    repo = Repo(
        name="edubot-testrepo",
